...
router.post('/create-org',upload.single('lib_file'),async (req,res,next)=>{
    ...
    if(org_exist) return res.status(400).send("Org name already exists");
        ...
        try {
            ...
            //reading xlsx file
            const wb = xlsx.readFile(path.join(__dirname,`../lib_uploads/${file_name}.xlsx`))
...
router.post('/send-otp/:org_mail',async (req,res)=>{
    ...
    sendMail(req.params.org_mail,subject,text);
    const date = new Date();
...
// fetch(`/delete-books/?mail=${bookid}&token=${token}`)/
router.delete('/delete-books/:id',org_verify,async(req,res)=>{
        try {
...
router.get('/pending/:org_id',org_verify,async (req,res)=>{
    try {
        const books = await bookings.find({isIssued:false,isAccepted:false,org:req.params.org_id }).populate('book','title author publication').populate('user','user_name mail ph_no').populate('org','org_name')
...
router.post('/pending',async (req,res)=>{
    ...
    try {
        ...
        // const subject = " Booking Acceptance"
        // const text = `Your booking for ${book.title} is accepted.You can go to library and take that book`
...
router.get('/not-returned',org_verify,async (req,res)=>{
    try {
        const books = bookings.find({isIssued:true,isReturned:false}).populate('book','title author publication').populate('user','user_name mail ph_no').populate('org','org_name')
        res.status(200).send([books[0].book.title,books[0].book.author,books[0].book.publication,books[0].user.user_name,books[0].user.mail,books[0].user.ph_no,books[0].org.org_name,books[0].issued_date,books[0].expire_date])
    } catch (error) {
        res.send(error.message)
    }
