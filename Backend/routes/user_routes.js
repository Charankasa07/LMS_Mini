...
router.get("/get-bookings-data/:user_name",verify,async(req,res)=>{
    ...
    const data1={};
    const data=await bookings.find({user:userexist._id}).populate('book','title').populate('org','org_name').then(p=>data1=p)
...
router.post('/otp-verification/:mail',async (req,res)=>{
    ...
    try {
        ...
        if(otp === data.otp){
            ...
            const token=jwt.sign({user_name:data.user_name},process.env.TOKEN_SECRET);
